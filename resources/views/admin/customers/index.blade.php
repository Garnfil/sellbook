...
@push('scripts')
    ...
    <script>
        function showTable() {
            window.LaravelDataTables['customer-details-table'].draw(false);
        }
        
        // For deleting data
        $('#customer-details-table').on('click', '.delete-btn', function (e) {
            let data_id = $(this).attr('data-id');
            renderSweetAlert({
                title: 'Are you sure you want to delete?',
                ajax: {
                    url: `{{ route('admin.customers.destroy', '') }}/${data_id}`,
                    method: "DELETE",
                    data: {
                        _token: "{{ csrf_token() }}",
                    },
